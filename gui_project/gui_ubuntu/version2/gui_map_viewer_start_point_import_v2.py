...
from robot_point_importer_v2 import dummy_x, dummy_y
...
def load_layers(folder: str, floor_code: str = "1F") -> dict:
    ...
    """
    layers = {}
    layers['base']    = gpd.read_file(os.path.join(folder, 'base_112.geojson'))
    layers['outline'] = gpd.read_file(os.path.join(folder, 'outline_112.geojson'))
    layers['doors']   = gpd.read_file(os.path.join(folder, 'doors_112_1.geojson'))
    stair_elev = gpd.read_file(os.path.join(folder, 'stair_n_elevator_112_1.geojson'))

    # id==8인 계단만
    layers['stair'] = stair_elev[stair_elev['id'] == 8].reset_index(drop=True)
    # id==7인 엘리베이터만
    layers['elevator'] = stair_elev[stair_elev['id'] == 7].reset_index(drop=True)

    # 층별 매장 + 계단 합치기
    shop_path = os.path.join(folder, f'shops_{floor_code}.geojson')
    if os.path.exists(shop_path):
        shops = gpd.read_file(shop_path)
        layers['room_n_stair'] = pd.concat([shops, layers['stair']], ignore_index=True)
    else:
        layers['room_n_stair'] = layers['stair']

    # 층별 path
    path_path = os.path.join(folder, f'path_{floor_code}.geojson')
...
class MapWidget(QWidget):
    ...
    def draw_layers(self):
        ...
        if rooms is not None and not rooms.empty:
            pen = QPen(QColor(50,100,150), 0.05)
            brush = QBrush(QColor(100,200,255,80))
