...
from gui_map_viewer_start_point_import_v2 import MapWidget  # 지도 위젯 임포트
...
class FloorGuidePage(QWidget):
    ...
    def goToFloor(self, floor_code: str):
        ...
        # 4) Base_page에서 geojson_folder, images_folder 가져오기
        base_page = self.stacked_widget.widget(1) if self.stacked_widget else None
        if base_page:
            # 5) MapWidget 생성 및 '로봇 네비게이션 버튼' 비활성화 플래그 설정
            map_w = MapWidget(
             base_page.geojson_folder,base_page.images_folder,floor_code=floor_code
             )
            map_w.allow_robot_nav = False
            map_w.view.scale(1.3, 1.3)
            self.detail_layout.addWidget(map_w)
