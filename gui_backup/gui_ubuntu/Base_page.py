...
class Basepage(QWidget):
    def __init__(self, stacked_widget=None):
        ...
        self.mqtt_client.connect_async("192.168.59.202", 1883, 60)
    ...
    def showFloorGuide(self):
        if self.stacked_widget:
            self.stacked_widget.setCurrentIndex(2)  # Floor_guide_page

            #항상 리스트 페이지로 전환
            floor_guide = self.stacked_widget.widget(2)
            if hasattr(floor_guide, "stack"):
                floor_guide.stack.setCurrentIndex(0)
    ...
    def refresh_to_base(self):
        ...
        for btn in self.floor_button_group.buttons():
            is_1f = btn.text() == "1F"
            btn.setChecked(is_1f)
            btn.setStyleSheet(self.getFloorButtonStyle(is_1f))
