...
from main_interface.srv import Battery
from main_interface.srv import Color
from main_interface.srv import SaveColor
from main_interface.srv import ResetOdom
from main_interface.srv import Onoff
from main_interface.srv import Calg
...
class OMOR1MiniNode(Node):
  def __init__(self):
    ...
    # ✅ 파라미터 선언 (타입과 기본값 명확히 지정)
    self.declare_parameter('port.name', '/dev/ttyUSB0')
    self.declare_parameter('port.baudrate', 115200)
    self.declare_parameter('motor.gear_ratio', 15.0)
    self.declare_parameter('motor.max_lin_vel_x', 0.6)
    self.declare_parameter('motor.max_ang_vel_z', 0.5)
    self.declare_parameter('wheel.separation', 0.57)
    self.declare_parameter('wheel.radius', 0.1)
    self.declare_parameter('sensor.enc_pulse', 60000.0)
    self.declare_parameter('sensor.use_gyro', False)

    # ✅ 파라미터 값 가져오기
    _port_name = self.get_parameter('port.name').value
    _port_baudrate = self.get_parameter('port.baudrate').value
    self.gear_ratio = self.get_parameter('motor.gear_ratio').value
    ...
    self.use_gyro = self.get_parameter('sensor.use_gyro').value
    self.max_lin_vel_x = self.get_parameter('motor.max_lin_vel_x').value
    self.max_ang_vel_z = self.get_parameter('motor.max_ang_vel_z').value
    ...
    # Set subscriber
    self.subCmdVelMsg = self.create_subscription(Twist, 'omo_cmd_vel', self.cbCmdVelMsg, 10)
